...
public class AccountRepository {
    ...
    private static final Map<Integer, Integer> db = IntStream.rangeClosed(1,10)
            .boxed().collect(Collectors.toConcurrentMap(Function.identity(), v->100));
    ...
    public static void deductBalance(int accountNumber, int amount){
        db.computeIfPresent(accountNumber, (k,v) -> v-amount);
    }

    public static void depositBalance(int accountNumber, int amount){
        db.computeIfPresent(accountNumber, (k,v) -> v+amount);
    }
